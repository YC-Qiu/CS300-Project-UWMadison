...
public class Camper implements Comparable<Camper> {
  ...
  /**
   * Constructor that sets the firstName, lastName, and age of an instance of
   * the camper class.
   * 
   * @param firstName - the first name of the camper
   * @param lastName -  the last name of the camper
   * @param age - the age of the camper
   * @throws IllegalArgumentException, if the age is outside of the range
   * [8,14] (inclusive).
   */
  ...
  /**
   * Accessor that returns both the first and last name of a Camper in the
   * correct format.
   * 
  ...
  /**
   * Method required to use the Comparable interface. Compares based on
   * "lastName, firstName" string lexigraphically.
   * 
   * @return 0 if they are the same, a negative int (<0) if this Camper is less
   * than the argument, a positive int (>0) if this Camper is greater than the
   * argument.
   */
  @Override
  public int compareTo(Camper camper) {
    String string1 = this.lastName + ", " + this.firstName;
    String string2 = camper.lastName + ", " + camper.firstName;
